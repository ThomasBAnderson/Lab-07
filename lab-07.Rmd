...
ggplot(data = evals, aes(x = score), na.rm = TRUE) +
         geom_histogram() +
         labs(x = "Score Rating", y = NULL)


```

The data is skewed towards the upper limits of Score Rating with more courses rated between around 4.6 than lower rated around 3. This tells us students are more likely to rate courses at a higher rating. This is what I expected as people tend to rate things higher than what they truly think.
...
score_bty_fit <- augment(score_bty_fit$fit)

ggplot(score_bty_fit, mapping = aes(x = .fitted, y = .resid)) +
  geom_jitter() +
